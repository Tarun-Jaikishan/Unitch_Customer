      ...
      api
        ...
        .then((resp) => {
          if (resp.data.success) {
            ...
            const open_ticket = d.filter(
              (item) =>
                item.status === OPEN_TICKET || item.status === PENDING_TICKET
            );
            const closed_ticket = d.filter(
              (item) => item.status === CLOSED_TICKET
            );
            this.setState({
              account_id,
              open_ticket,
      ...
      <>
        <div>
          <div className="d-flex justify-content-center">
            {this.state.open_ticket.length === 0 && (
              <button
        ...
        <div className="d-flex justify-content-center p-5 w-full">
          <div>
            {this.state.open_ticket.map((item) => (
              <TicketDetails
                {...item}
                display_ticket={this.displayReply}
                key={item.id}
              />
            ))}

            {this.state.closed_ticket.map((item) => (
              <TicketDetails
                {...item}
                display_ticket={this.displayReply}
                key={item.id}
          ...
          {/* <div className="p-5 mt-3">
            {Object.keys(this.state.display_ticket).length > 0 && (
              <DisplayTicket ticket={this.state.display_ticket} />
            )}
          </div> */}
        </div>
      </>
