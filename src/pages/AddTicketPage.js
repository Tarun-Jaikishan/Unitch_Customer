...
class AddTicketPage extends Component {
  ...
  componentDidMount() {
    ...
    if (token) {
      const url =
        "complaint-subcategory/list?filter[status]=1&fields=id,name,category_id";
      const headers = {
  ...
  handleSubmit = (e) => {
    ...
    if (this.isFormValid()) {
      const sub_cat = this.state.sub_category_list.filter(
        (i) => i.id === parseInt(this.state.form.sub_category.value)
      );
      const request = {
    ...
    content = (
      <div className="card">
        <div className="px-5 py-3">
          <strong className="text-xl d-flex flex-gap align-items-center ">
            <Link
              to="/myaccount"
              type="button"
        ...
        <div className="d-flex justify-content-center px-5 pb-5">
          <Form className="w-half" onSubmit={this.handleSubmit}>
            <Form.Group controlId="form.sub_category.value">
              ...
              >
                ...
                {this.state.sub_category_list.map((item, index) => (
