...
export const autoLogin = () => {
  return (dispatch) => {
    ...
    if (token) {
      ...
      api
        .post("/user/login-token", authData, { headers: headers })
        .then((response) => {
...
const setToken = async (response, dispatch) => {
  ...
  localStorage.setItem(
    USER_TOKEN,
    JSON.stringify({ token: data.access_token, time: new Date() })
  );
  localStorage.setItem(USER_DETAILS_STORAGE, JSON.stringify(user_data));
  localStorage.setItem(
    USER_AUTH_TOKEN,
    JSON.stringify({ token: data.auth_token, time: new Date() })
  );
...
export const auth = (username = "", password = "", isUserSignin = false) => {
  return (dispatch) => {
    ...
    if (isUserSignin) {
      ...
      api
        .post("/user/login-token", tokenRequest, { headers: headers })
        .then((response) => {
          let data = response.data.data;
          let user_data = {
            id: data.id,
